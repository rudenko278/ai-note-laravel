...
class AdminController extends Controller
{
    ...
    public function paymentPlansSave(Request $request){
        ...
        try{
            // error_log("AdminController -> paymentPlansSave()");
            // saveGatewayProducts($planId, $productName, $price, $frequency, $type)
            $tmp = PaymentController::saveGatewayProducts($plan->id, $request->name, (int)(((double)$request->price)*100), $request->frequency, $request->type);
        }catch(\Exception $ex){
